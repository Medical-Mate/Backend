    ...
    @Operation(
            ...
            description = """
                    ...
                    **검증에 걸린 값은 `unknown` 으로 저장되고 `rejectedFields` 에 이름이 담깁니다.**
                    ...
                    서버가 보는 규칙: 축 값 80자, 질문 최대 5개·각 200자, 진료과는 정해진 13개 목록.
    ...
    @Operation(
            ...
            description = """
                    ...
                    **같은 문답에서 나온 카드는 버전을 가리지 않고 전부 지워집니다.** 환자에게는
                    ...
                    - **진료 기록은 남고 연결만 끊깁니다** — 카드는 진료 전에 만든 준비물이고
                      기록은 진료에서 실제로 들은 것입니다. 준비물을 지웠다고 의사에게 들은
                      말이 사라지면 안 됩니다. 기록만 지우려면 `DELETE /api/visits/{id}` 를
                      쓰세요. 그 기록의 `cardId` 는 `null` 이 되고 `cardTitle` 은 남습니다
                    - **일정도 남고 연결만 끊깁니다** — 카드를 지웠다고 병원 예약까지 사라지면
                      환자가 진료를 놓칩니다
